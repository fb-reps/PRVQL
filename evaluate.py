...
def validate_model_predictions(model_predictions, test_annotations):
    ...
    print(len(video_predictions), len(video_annotations),111111)
...
def evaluate(gt_file, pred_file):
    ...
    for pair_name, metrics in pair_metrics.items():
        ...
        with open(os.path.join('/mnt/DATA/bingfan/code/VQL_updatequery','val_process.txt'), 'a') as file:
            for pair_name, metrics in pair_metrics.items():
                file.write("-" * 20 + "\n")
                file.write(pair_name + "\n")
                file.write("-" * 20 + "\n")
                
                # Modify the metrics dictionary
                metrics = {
                    "tAP": metrics["Temporal AP                    @ IoU=0.25:0.95"],
                    "tAP @ IoU=0.25": metrics["Temporal AP                    @ IoU=0.25     "],
                    "stAP": metrics["SpatioTemporal AP              @ IoU=0.25:0.95"],
                ...
                for k, v in metrics.items():
                    file.write(f"{k:<20s} | {v:>10.3f}\n")

                file.write("\n")  # Add an extra newline between entries

        print("Metrics have been appended to val_process.txt.txt")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--gt-file", required=False, type=str, default='/mnt/DATA-1/bingfanprojects/datasets/VQ2D/episodic-memory/VQ2D/datav2/vq_val_random100.json')
    parser.add_argument("--pred-file", required=False, type=str, default='/home/UNT/bf0191/Documents/vqloc_new/VQLoCOnlyUpdatequery/output/ego4d_vq2d/val/validate/inference_cache_val_results.json.gz')
