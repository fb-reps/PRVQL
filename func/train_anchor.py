...
from utils import exp_utils, train_utils, vis_utils
from dataset import dataset_utils
from einops import rearrange
from utils.loss_utils import GiouLoss, calculate_total_loss
...
def train_epoch(config, loader, model, optimizer, schedular, scaler, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    for batch_idx, sample in enumerate(loader):
        ...
        if config.train.use_query_roi and 'query_frame' in sample.keys():
            last_result, results = model(clips, 
                          sample['query_frame'], 
                          query_frame_bbox=sample['query_frame_bbox'], 
                          training=True, fix_backbone=config.model.fix_backbone)
        else:
            last_result, results = model(clips, queries, training=True, fix_backbone=config.model.fix_backbone)
        ...
        total_loss, loss_meters, losses, preds_top, results_vis = calculate_total_loss(sample, results, config, loss_meters)
        ...
        if iter_num % config.vis_freq == 0 and rank == 0:
            ...
            vis_utils.vis_pred_scores(sample=sample,
                                    ...
                                    output_dir=output_dir,
                                    subfolder='train')
            vis_utils.vis_pred_topk(sample=sample,
                                    pred=last_result,
                                    iter_num=iter_num,
                                    output_dir=output_dir,
                                    subfolder='topk')
            
            vis_utils.vis_feature(sample=sample,
                                    pred=last_result,
                                    iter_num=iter_num,
                                    output_dir=output_dir,
                                    subfolder='feature')
...
def validate(config, loader, model, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(loader):
            ...
            clips, queries = sample['clip'], sample['query']
            if config.train.use_query_roi and 'query_frame' in sample.keys():
                result,_ = model(clips, 
                            sample['query_frame'], 
                            query_frame_bbox=sample['query_frame_bbox'], 
                            training=False, fix_backbone=config.model.fix_backbone)
            else:
                result,results = model(clips, queries, training=False, fix_backbone=config.model.fix_backbone)
            
            results_vis=[]
            for pred_id, pred in enumerate(results):
                loss, preds_top = val_performance(config, pred, sample)
                results_vis.append(preds_top)
