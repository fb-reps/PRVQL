...
def calculate_total_loss(sample, results, config, loss_meters):
    total_loss = 0.0
    length=len(results)
    results_vis=[]
    stage_balance=[4,2,1]

    for preds_id, preds in enumerate(results):
        losses, preds_top, sample = get_losses_with_anchor(config, preds, sample)
        results_vis.append(preds_top)
        for k, v in losses.items():
            if 'loss' in k:
                total_loss += losses[k.replace('loss_', 'weight_')] * v * stage_balance[preds_id]

                # Record loss value
                if preds_id==length-1:
                    loss_meters.add_loss_value(k, v.detach().item())

    # Average loss based on cumulative steps
    total_loss /= config.train.accumulation_step
    total_loss /= length

    return total_loss, loss_meters, losses, preds_top, results_vis
